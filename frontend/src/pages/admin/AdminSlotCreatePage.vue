...
async function onCreate() {
  if (!createForm.value.specialistId) {
    showAlertModal({
      type: "error",
      message: "Please select a specialist for the new slot.",
    });
    return;
  }
  if (
  ...
  if (!isValidTimeRange(createForm.value.start, createForm.value.end)) {
    showAlertModal({
      type: "error",
      message: "Create slot start time must be earlier than end time.",
    });
    return;
  }
  ...
  try {
    ...
    showAlertModal({
      type: "error",
      message: e?.message || "Failed to create slot",
    });
  } finally {
  ...
  <section class="page">
    <header class="page__header">
      <div>
        <h1>Create Slot</h1>
        <p class="subtitle">
          Create consultation slots on a dedicated page for a cleaner workflow.
        </p>
      </div>
      <button
        type="button"
        class="btn-neutral"
        @click="router.push({ name: 'admin.slots' })"
      >
