...
const hasResultList = computed(
  () => resultMode.value === "list" && quoteResults.value.length > 0,
);
const isListMode = computed(() => resultMode.value === "list");
const resultCountLabel = computed(
  () =>
    `${quoteResults.value.length} result${quoteResults.value.length === 1 ? "" : "s"}`,
);
const ruleCountLabel = computed(
  () => `${rules.value.length} rule${rules.value.length === 1 ? "" : "s"}`,
);
...
const quoteSpecialist = computed(() =>
  buildSpecialistLabel(
    quote.value?.specialistId ??
      resolvedSpecialistId.value ??
      specialistId.value.trim(),
...
const quoteDuration = computed(() => {
  ...
  const value =
    quote.value?.duration ?? (Number.isFinite(fallback) ? fallback : null);
...
const formattedAmount = computed(() =>
  formatCurrency(quoteAmount.value, quoteCurrency.value),
);
const quoteSummary = computed(() =>
  buildQuoteSummary(quoteDuration.value, quoteType.value),
);
...
function formatTypeLabel(value) {
  const normalized = String(value || "")
    .trim()
    .toLowerCase();
  const match = sessionTypeOptions.find(
...
function buildQuoteSummary(durationValue, typeValue) {
  ...
  if (!Number.isFinite(mins) || mins <= 0)
    return `Consultation session${typeText}`.trim();
  if (mins % 60 === 0) {
    const hours = mins / 60;
...
function sortQuoteResults(rows) {
  return [...rows].sort((a, b) => {
    ...
    return String(a.specialistId || "").localeCompare(
      String(b.specialistId || ""),
    );
  });
}
...
function buildQuoteCombinations() {
  ...
  const selectedType = hasType.value
  ...
  const types = selectedType
    ? [selectedType]
    : sessionTypeOptions.map((option) => option.value);
...
async function onQuote() {
  ...
  try {
    const selectedSpecialist = specialists.value.find(
    ...
    if (hasDuration.value && hasType.value) {
      ...
      if (!firstRecord) {
        error.value =
          "No quote available for the selected specialist and filters";
    ...
    if (!normalizedResults.length) {
      ...
      error.value =
        ...
        "No quote combinations available for this specialist";
...
function buildRuleSearchParams() {
  const params = {};
  if (ruleSearchForm.value.specialistId)
    params.specialistId = ruleSearchForm.value.specialistId;
  if (
    ruleSearchForm.value.duration !== "" &&
    Number.isFinite(Number(ruleSearchForm.value.duration))
  ) {
...
async function loadRules(options = {}) {
  ...
  try {
    ...
    if (announceSuccess) {
      showAlertModal({
        type: "success",
        message: `Loaded ${ruleCountLabel.value}.`,
      });
    }
  } catch (e) {
    rules.value = [];
    ...
    showAlertModal({
      type: "error",
      message: e?.message || "Failed to load pricing rules",
    });
  } finally {
...
async function onUpdateRule() {
  ...
  if (
    !Number.isFinite(Number(editForm.value.duration)) ||
    Number(editForm.value.duration) <= 0
  ) {
    showAlertModal({
      type: "error",
  ...
  try {
    await api.adminUpdatePricingRule(
      editForm.value.id,
      buildRulePayload(editForm.value),
    );
    await loadRules();
    showAlertModal({
      type: "success",
      message: `Pricing rule ${editForm.value.id} updated successfully.`,
    });
    closeEdit();
  } catch (e) {
    showAlertModal({
      type: "error",
      message: e?.message || "Failed to update pricing rule",
    });
  } finally {
...
async function onDeleteRule(row) {
  ...
  if (!id) {
    showAlertModal({
      type: "error",
      message: "This pricing rule is missing an ID and cannot be deleted.",
    });
    return;
  }

  const confirmed = window.confirm(
    `Delete pricing rule "${id}"? This action cannot be undone.`,
  );
  if (!confirmed) return;
  ...
  try {
    ...
    showAlertModal({
      type: "success",
      message: `Pricing rule ${id} deleted successfully.`,
    });
  } catch (e) {
    showAlertModal({
      type: "error",
      message: e?.message || "Failed to delete pricing rule",
    });
  } finally {
...
<template>
  <section class="page">
    <header class="page__header">
      <h1>Pricing Rules</h1>
      <p class="subtitle">Manage reusable pricing rules for specialists.</p>
    </header>
    ...
    <section class="calc-card list-card">
      <div class="list-toolbar">
        <div class="toolbar-title">
          ...
          <p class="list-note">
            Review all saved pricing rules, search by specialist or condition,
        ...
        <div class="toolbar-actions">
          <button
            ...
            @click="router.push({ name: 'admin.pricingCreate' })"
          >
          ...
          <button
            ...
            @click="loadRules({ announceSuccess: true })"
          >
        ...
        <section class="search-card">
          <div class="field-grid field-grid--three">
            <label class="field">
              ...
              <select
                v-model="ruleSearchForm.specialistId"
                class="input input--select"
                :disabled="specialistsLoading"
              >
                <option value="">All specialists</option>
                <option
                  v-for="row in specialists"
                  :key="row.id"
                  :value="row.id"
                >
            ...
            <label class="field">
              ...
              <input
                v-model="ruleSearchForm.duration"
                type="number"
                min="1"
                step="1"
                class="input"
      ...
      <div v-if="rulesLoading && !rules.length" class="state">
        Loading pricing rules...
      </div>
      <div v-else-if="!rules.length" class="state state--empty">
        {{
          searchedRulesOnce
            ? "No pricing rules found for the current filters."
            : "Run a search to review pricing rules."
        }}
      </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="row in filteredRules" :key="ruleId(row)">
              ...
              <td class="cell--detail" :title="ruleDetail(row)">
                {{ ruleDetail(row) }}
              </td>
              <td>
