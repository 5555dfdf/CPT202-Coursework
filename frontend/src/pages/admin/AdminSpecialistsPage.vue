...
// create, search, batch status, edit, delete
...
const filteredExpertiseOptions = computed(() => {
  ...
  return (expertiseList.value || []).filter((item) =>
    String(item?.name ?? "")
      .toLowerCase()
      .includes(q),
  );
...
const selectedExpertise = computed(() =>
  (form.value.expertiseIds || []).map(
    (id) => expertiseMap.value.get(String(id)) || { id, name: id },
  ),
);
const selectedEditExpertise = computed(() =>
  (editForm.value.expertiseIds || []).map(
    (id) => expertiseMap.value.get(String(id)) || { id, name: id },
  ),
);
...
const totalSpecialists = computed(() =>
  Math.max(Number(page.value?.total) || 0, (page.value?.items || []).length),
);
...
const showNameHelper = computed(() => {
  return (
    nameFocused.value || !!form.value.name.length || !!nameLimitError.value
  );
});
...
const expertisePlaceholder = computed(() => {
  ...
  if (form.value.expertiseIds.length === 1)
    return selectedExpertise.value[0]?.name || "1 selected";
...
const editExpertisePlaceholder = computed(() => {
  ...
  if (editForm.value.expertiseIds.length === 1)
    return selectedEditExpertise.value[0]?.name || "1 selected";
...
function removeExpertise(id) {
  const targetId = String(id);
  form.value.expertiseIds = normalizeExpertiseIds(
    form.value.expertiseIds,
...
function removeEditExpertise(id) {
  const targetId = String(id);
  editForm.value.expertiseIds = normalizeExpertiseIds(
    editForm.value.expertiseIds,
  ).filter((itemId) => itemId !== targetId);
}
...
async function loadSpecialists() {
  ...
  try {
    ...
    const currentIds = new Set(
      (page.value?.items || []).map((item) => String(item?.id || "")),
    );
...
function toggleSelectAll(event) {
  ...
  selectedIds.value = filteredSpecialists.value
...
async function onBatchSetStatus(status) {
  if (!selectedIds.value.length) {
    showAlertModal({
      type: "error",
      message: "Please select at least one specialist.",
    });
    return;
  }
...
async function onCreate() {
  ...
  try {
    const price =
      form.value.price === "" ? undefined : Number(form.value.price);
...
async function onSaveEdit() {
  ...
  try {
    const price =
      editForm.value.price === "" ? undefined : Number(editForm.value.price);
    ...
    if (editForm.value.status !== editOriginalStatus.value) {
      const statusUpdated = await api.adminSetSpecialistStatus(
        editForm.value.id,
...
watch(
  ...
  () => {
    const visibleIds = new Set(
      filteredSpecialists.value.map((item) => String(item?.id || "")),
    );
...
<template>
  <section class="page">
    <header class="page__header">
      ...
      <p class="subtitle">
        Create and manage specialist profiles, pricing, and expertise
        assignments.
      </p>
    </header>
    ...
    <section class="calc-card create-card">
      ...
      <div class="create-grid">
        ...
        <label class="field field--full">
          ...
          >
            <p class="limit-helper__text">
              {{ bioLimitError || `Maximum ${BIO_MAX_WORDS} words` }}
            </p>
            <p class="limit-helper__count">
              {{ bioWordCount }}/{{ BIO_MAX_WORDS }} words
            </p>
          </div>
        </label>
      </div>
      ...
      <div ref="expertiseFieldRef" class="field expertise-field field--full">
        ...
        >
          ...
          <span class="multi-trigger__arrow">{{
            expertiseOpen ? "^" : "v"
          }}</span>
        </button>
        ...
        <div v-if="expertiseOpen" class="multi-panel">
          ...
          <div class="multi-options">
            <button
              ...
              class="multi-option"
              :class="{
                'multi-option--active': form.expertiseIds.includes(
                  String(item.id),
                ),
              }"
            ...
            >
              ...
              <span class="multi-option__state">
                {{
                  form.expertiseIds.includes(String(item.id))
                    ? "Selected"
                    : "Select"
                }}
              </span>
    ...
    <section class="calc-card list-card">
      ...
      <div class="toolbar-meta">
        ...
        <button
          type="button"
          class="btn-neutral toolbar-meta-btn"
          :disabled="batchLoading"
          @click="onBatchSetStatus('Active')"
        >
          Set Active
        </button>
        <button
          ...
          class="btn-neutral toolbar-meta-btn"
          :disabled="batchLoading"
          @click="onBatchSetStatus('Inactive')"
        >
          Set Inactive
        </button>
      </div>

      <div v-if="loading && !(page.items || []).length" class="state muted">
        Loading...
      </div>
      ...
      <div v-else-if="!filteredSpecialists.length" class="state muted">
        {{
          searchQuery
            ? "No specialist matched your search."
            : "No specialist data."
        }}
      </div>
        ...
        <table class="table">
              ...
              <th scope="col" class="th-select">
                <input
                  ...
                  :checked="
                    ...
                    selectedIds.length === filteredSpecialists.length
                  "
          ...
          <tbody>
            <tr v-for="s in filteredSpecialists" :key="s.id">
              ...
              <td>
                <div
                  v-if="specialistExpertiseNames(s).length"
                  class="expertise-summary"
                >
                  <span
              ...
              <td>
                <span :class="specialistStatusClass(s)">{{
                  specialistStatus(s)
                }}</span>
              </td>
              <td>
      ...
      <section class="modal-card">
        ...
        <div class="create-grid">
          ...
          <label class="field field--full">
            ...
            >
              <p class="limit-helper__text">
                {{ editBioLimitError || `Maximum ${BIO_MAX_WORDS} words` }}
              </p>
              <p class="limit-helper__count">
                {{ editBioWordCount }}/{{ BIO_MAX_WORDS }} words
              </p>
            </div>
          </label>
        </div>
          ...
          <div v-if="editExpertiseOpen" class="multi-panel">
            ...
            <div class="multi-options">
              <button
                ...
                class="multi-option"
                :class="{
                  'multi-option--active': editForm.expertiseIds.includes(
                    String(item.id),
                  ),
                }"
              ...
              >
                ...
                <span class="multi-option__state">
                  {{
                    editForm.expertiseIds.includes(String(item.id))
                      ? "Selected"
                      : "Select"
                  }}
                </span>
              ...
              <div
                v-if="!filteredEditExpertiseOptions.length"
                class="multi-empty"
              >
                No expertise matched your search.
              </div>
        ...
        <div class="field">
          ...
          <div class="option-row">
            <button
              ...
              :class="{
                'option-btn--active-positive': editForm.status === 'Active',
              }"
            ...
            <button
              ...
              :class="{
                'option-btn--active-negative': editForm.status === 'Inactive',
              }"
