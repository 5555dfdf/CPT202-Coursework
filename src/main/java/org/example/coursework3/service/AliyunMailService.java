...
public class AliyunMailService {
    ...
    @Async
    public void sendBookingStatusNotification(String toAddress, String status, String note) throws Exception {
        String subject = "Your Booking Result";
        String content = status.equalsIgnoreCase("Confirmed") ? "Confirmed" : "Rejected";
        String bodyHtml = "<h3>Booking Handling Info</h3>" +
                "<p>Your Booking Result：<strong>" + content + "</strong></p>";

        if (note != null && !note.isEmpty()) {
            bodyHtml += "<p>Note：" + note + "</p>";
        }

        bodyHtml += "<p>Please log in to the system to view details.</p>";
